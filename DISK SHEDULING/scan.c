#include<stdio.h>
void main(){
    int n,head,size,seek_time=0,pos;
    ...
    if(pos>0){
        seek_time+=size-1 - a[pos-1];
        printf("\nHead moves from %d to %d: %d",size-1,a[pos-1],size-1 - a[pos-1]);

        for(int i=pos-1;i>0;i--){
            seek_time+=a[i]-a[i-1];
            printf("\nHead moves from %d to %d: %d",a[i],a[i-1],a[i]-a[i-1]);
        }
